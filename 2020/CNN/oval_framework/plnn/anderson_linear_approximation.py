...
class AndersonLinearizedNetwork(LinearizedNetwork):
    ...
    def define_linear_approximation(self, input_domain, int_bounds_provided=False, compute_final_layer=True,
        ...
        if not self.model_built:
            ...
            if self.mode == "mip-exact":
                # Shut off Gurobi's cut generation.
    ...
    def compute_pre_activation_bounds(self, x_idx, layer, x_km1_vars, ub_only=False, counterexample_verification=False,
        ...
        self.n_opt_neurons = 2 * prod(out_shape)  # no. of neurons over which we're optimizing
            ...
            for neuron_idx in neurons:
                ...
                if bound_diff < self.bounds_num_tolerance:
                    ...
                    if c_lb + self.bounds_num_tolerance >= 0:
                        c_lb = max(0, c_lb)
                if not self.model_built:
                    lin_expr = self.get_layer_linear_expression(layer, x_km1_vars, neuron_idx)
    ...
    def optimize_var(self, var, gurobi_callback, allowed_statuses, store_input=False):
        ...
        if self.mode in ["lp-cut", "lp-all"]:
            ...
            if self.model.status not in allowed_statuses + [grb.GRB.NODE_LIMIT, grb.GRB.INTERRUPTED]:
                raise RuntimeError(f"MIP crashed with status code {self.model.status}")

        if self.model.status in [grb.GRB.INFEASIBLE, grb.GRB.INF_OR_UNBD]:  # (infeasible)
            out = float('inf')
            if store_input:
    ...
    @staticmethod
    def get_layer_linear_expression(layer, x_km1_vars, out_coordinates, bias_factor=1):
        """
        ...
        :param bias_factor: a multiplicative factor for the bias. Not used in this relaxation (i.e., set to 1), but
        employed for instance in the extended relaxation.
        :return: the Gurobi linear expression
        """

        if type(layer) is nn.Linear:
            neuron_idx = out_coordinates
            lin_expr = layer.bias[neuron_idx].item() * bias_factor
            ...
            # Compute W_k * x_{k-1} + b_k, as a Gurobi linear expression.
            lin_expr = layer.bias[out_chan_idx].item() * bias_factor
    ...
    def insert_tighten_cut(self, model, where, first_call=False, counterexample_verification=False):
        ...
        if where == grb.GRB.Callback.MIPNODE or where == "lp-cut":
            ...
            if where == "lp-cut" or model.cbGet(grb.GRB.Callback.MIPNODE_STATUS) == grb.GRB.Status.OPTIMAL:
                ...
                # TODO: pass the branching scores to this and add in the scoring order?
                ...
                if self.applied_cuts < self.cut_treshold:
                    for layer_idx in range(self.last_relu_index+1):
                        ...
                        if type(layer) is nn.ReLU:
                            ...
                            if type(previous_layer) is nn.Linear:
                                ...
                                for out_chan_idx in range(sample_out.size(1)):
                                    non_zero_indices = torch.nonzero(previous_layer.weight[out_chan_idx, :, :, :]).tolist()
                                    non_zero_indices_set = set([tuple(ind_list) for ind_list in non_zero_indices])
                                    for out_row_idx in range(sample_out.size(2)):
                                        for out_col_idx in range(sample_out.size(3)):
                ...
                # TODO: move to different strategies for inserting cuts, like k most violated constraints?
