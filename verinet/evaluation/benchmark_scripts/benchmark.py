...
def run_benchmark(images: np.array,
    ...
    Args:
        ...
        mean:
            The normalising means
        std:
            The normalising stds
        timeout:
        ...
        targets:
            ...
            correct classes.
        max_procs:
            The maximum number of processes used.
        max_val:
            The maximum pixel value (Usually 1 or 255)
        gradient_step:
            The step size used in the gradient-descent local search.
    """

    # Get the Gurobi academic license print at the top
    import gurobipy
    gurobipy.Model()
    ...
    with open(result_path, 'w', buffering=1) as f:
        ...
        solver = VeriNet(model,
                         ...
                         gradient_descent_step=gradient_step,
        ...
        for eps in epsilons:
            ...
            for i in tqdm(range(len(images))):
                ...
                pred_i = model(torch.Tensor((data_i - mean)/std)).argmax(dim=1).numpy()[0]
                ...
                input_bounds[..., 0] = (np.clip((data_i - eps), 0, max_val) - mean) / std
                input_bounds[..., 1] = (np.clip((data_i + eps), 0, max_val) - mean) / std
