...
def convert():
    ...
    outfile_dir = os.path.join(os.path.dirname(__file__), "../times/")
    ...
    for result_file in result_files:
        ...
        status, times, _, timeout = read_result_file(os.path.join(results_path, result_file))

        with open(os.path.join(outfile_dir, result_file), "w") as out_file:

            for i, time in enumerate(times):

                if status[i] == "Misclassified":
                    continue

                if float(time) < timeout:
                    out_file.write(f"{time:.2f}\n")
                else:
                    out_file.write(f"-\n")
